...
export const uploaImgNotification = async (img) => {
	// Create firebase reference
	const [originalName, ext] = img.originalname.split('.'); // -> [pug, jpg]

	const filename = `Notification/${originalName}-${Date.now()}.${ext}`;
	const imgRef = ref(storage, filename);
...
export const deleteImgNotification = async (url) => {
	try {
		console.log(url)
		const imgRef = ref(storage, url);
		console.log(imgRef)
...
export const updateImgNotification = async (img, url) => {
    try {
        // Eliminar la imagen antigua
        await deleteImgNotification(url);
        
        // Subir la nueva imagen
        const newPath = await uploaImgNotification(img);
...
export const getNotificationImgUrl = async notification => {
	const imgRef = ref(storage, notification.urlImagen);
	const imgUrl = await getDownloadURL(imgRef);
	notification.urlImagen = imgUrl;
	return notification
}